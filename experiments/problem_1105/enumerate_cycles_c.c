...
/*
 * Exact search for AR(n, C_k), 3 <= k <= n, without using the published
 ...
 * already has a fully assigned rainbow C_k or cannot improve the incumbent
...
enum {
    ...
    /* There are 1172 undirected labeled cycles of orders 3,...,7 in K_7. */
    MAX_CYCLES = 1200,
};

_Static_assert(MAX_EDGES <= 32, "color masks require at most 32 edges");
_Static_assert(MAX_CYCLES <= UINT16_MAX,
               "cycle indices must fit in uint16_t");

static const char *const DEFAULT_OUTPUT =
    "experiments/problem_1105/results_cycles_c.json";
...
typedef struct {
    ...
    int cycle_count;
    int cycle_count_by_k[MAX_N + 1];
    uint16_t completed_cycles[MAX_EDGES][MAX_CYCLES];
    int completed_cycle_count[MAX_EDGES];
...
static void usage(FILE *stream, const char *program)
{
    fprintf(stream,
            ...
            "Exact formula-free search for AR(n, C_k), 3 <= k <= n <= %d.\n",
            program, MAX_N);
}
...
static void store_cycle(Search *search, int k, const int order[MAX_N])
{
    /* Fix the least vertex first to remove rotations. */
    for (int i = 1; i < k; ++i) {
        if (order[i] < order[0]) {
            return;
        }
    }
    /* Keep one of the two orientations. */
    if (order[1] > order[k - 1]) {
        return;
    }
    if (search->cycle_count >= MAX_CYCLES) {
        fail("internal cycle capacity exceeded");
    }

    int cycle_index = search->cycle_count++;
    Cycle *cycle = &search->cycles[cycle_index];
    cycle->k = (uint8_t)k;
    cycle->edge_count = (uint8_t)k;
    int completion_edge = -1;
    for (int i = 0; i < k; ++i) {
        int edge = search->edge_index[order[i]][order[(i + 1) % k]];
        if (edge < 0) {
            fail("internal edge lookup failure");
        }
        cycle->edges[i] = (uint8_t)edge;
        if (edge > completion_edge) {
            completion_edge = edge;
        }
    }
    ++search->cycle_count_by_k[k];

    int bucket_size = search->completed_cycle_count[completion_edge];
    if (bucket_size >= MAX_CYCLES) {
        fail("internal completed-cycle capacity exceeded");
    }
    search->completed_cycles[completion_edge][bucket_size] =
        (uint16_t)cycle_index;
    search->completed_cycle_count[completion_edge] = bucket_size + 1;
}

static void generate_cycles_recursive(Search *search, int k, int depth,
                                      unsigned used_vertices,
                                      int order[MAX_N])
{
    if (depth == k) {
        store_cycle(search, k, order);
        return;
    }
    for (int vertex = 0; vertex < search->n; ++vertex) {
        ...
        order[depth] = vertex;
        generate_cycles_recursive(search, k, depth + 1,
...
static void initialize_cycles(Search *search)
{
    int order[MAX_N] = {0};
    for (int k = 3; k <= search->n; ++k) {
        generate_cycles_recursive(search, k, 0, 0U, order);
        int expected = choose(search->n, k) * factorial(k - 1) / 2;
        if (search->cycle_count_by_k[k] != expected) {
            fail("internal cycle-count check failed");
        }
    }
}

static bool cycle_is_rainbow(const Search *search, const Cycle *cycle)
{
    uint32_t used_colors = 0U;
    for (int i = 0; i < cycle->edge_count; ++i) {
        uint32_t bit = 1U << search->coloring[cycle->edges[i]];
...
static uint32_t remove_completed_rainbow_cycles(const Search *search,
                                                 int assigned_edge,
                                                 uint32_t active)
{
    int count = search->completed_cycle_count[assigned_edge];
    for (int i = 0; i < count && active != 0U; ++i) {
        const Cycle *cycle =
            &search->cycles[search->completed_cycles[assigned_edge][i]];
        uint32_t bit = 1U << (cycle->k - 3);
        if ((active & bit) != 0U && cycle_is_rainbow(search, cycle)) {
            active &= ~bit;
        }
    }
    return active;
...
static void print_progress(Search *search)
{
    ...
    for (int k = 3; k <= search->n; ++k) {
        printf(" AR(n,C_%d)=%d", k, search->best[k]);
    }
    putchar('\n');
    fflush(stdout);
    do {
                             ...
                             int number_of_colors, uint32_t active)
{
    ...
    for (int pass = 0; pass < 2; ++pass) {
        ...
        for (int color = first; color < limit; ++color) {
            ...
            uint32_t child_active = remove_completed_rainbow_cycles(
...
static Result run_search(int n, uint64_t progress_every)
{
    ...
    for (int k = 3; k <= n; ++k) {
        result.cycle_count_by_k[k] = search.cycle_count_by_k[k];
        result.best[k] = search.best[k];
        memcpy(result.witness[k], search.witness[k],
               (size_t)search.edge_count);
    }

    printf("n=%d: cycles=%d, nodes=%" PRIu64 ", terminal=%" PRIu64
           ", pruned=%" PRIu64,
           n, search.cycle_count, result.nodes, result.terminal_colorings,
           result.pruned_subtrees);
    for (int k = 3; k <= n; ++k) {
        printf(", AR(n,C_%d)=%d", k, result.best[k]);
    }
    putchar('\n');
    return result;
}
                              ...
                              int result_count)
{
    if (fprintf(stream,
                ...
                "with incremental rainbow-cycle and incumbent-bound "
                "pruning\",\n"
                "  \"values\": [\n") < 0) {
        return false;
    }
    ...
    for (int i = 0; i < result_count; ++i) {
        ...
        for (int k = 3; k <= result->n; ++k) {
            ...
            fprintf(stream,
                    "        {\"k\": %d, \"anti_ramsey\": %d, "
                    "\"cycle_count\": %d, \"witness_color_classes\": ",
                    k, result->best[k], result->cycle_count_by_k[k]);
